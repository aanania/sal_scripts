...
def main(args):
    ...
    mgr = SALPY_ATPtg.SAL_ATPtg()
    mgr.salCommand("ATPtg_command_start")
    mgr.salCommand("ATPtg_command_enable")
    mgr.salCommand("ATPtg_command_disable")
    mgr.salCommand("ATPtg_command_standby")
    mgr.salCommand("ATPtg_command_raDecTarget")
    mgr.salTelemetrySub("ATPtg_timeAndDate")

    def disable():
        log.debug('Disabling PT Kernel...')
        disable = SALPY_ATPtg.ATPtg_command_disableC()
    ...
    def standby():
        ...
        standby = SALPY_ATPtg.ATPtg_command_standbyC()
    ...
    target_data = SALPY_ATPtg.ATtg_command_raDecTargetC()
    target_data.frame = 1
    ...
    time_data = SALPY_ATPtg.ATPtg_timeAndDateC()
    ...
    try:
        ...
        while retval < 0:
            ...
            if (time.time() - time_start) > args.timeout:
                raise IOError('Can not get time and date from ATPtg.')
